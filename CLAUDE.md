...
**포지셔닝**: "30분 걸리던 후기 글을 5분 만에, 근데 내가 쓴 것처럼"
**타겟**: 맛집/카페 후기를 꾸준히 올리는 개인 블로거, 협찬 후기 작성이 필요한 소상공인

**확정된 MVP 범위 (심플화)**
- ✅ 말투 샘플 3개 업로드 → 스타일 학습
- ✅ 정보 입력 폼 (위치, 영업시간, 메뉴, 가격 등)
- ✅ 사진 업로드 → Claude Vision이 사진 내용 파악 → 텍스트에 자연스럽게 반영
- ✅ 완성 텍스트 + [사진 N] 마커 출력 (복붙용)
- ❌ 사진 분류 확인/수정 UI (제거 — 자동 처리)
- ❌ HTML 미리보기 (제거 — 복잡도 대비 가치 낮음)
- ❌ 네이버 자동 발행 (API 없음)
